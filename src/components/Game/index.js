        ...
        <h2>Коллекция пользователя sever79 ({games.length} игр)</h2>
        {
          ...
          <ul className="list">
            {Object.keys(games).map(game => (
              <Card as="li" key={game} className="item">
                <div className="imgWrapper"><img src={games[game].thumbnail._text} className="img" alt={games[game].thumbnail._text} /></div>
                <div className="info">
                  <div className="name">
                    <a href={'https://boardgamegeek.com/boardgame/' + games[game]._attributes.objectid + '/'} target="_blank" rel="noopener noreferrer">{games[game].name._text}</a>
                  </div>
